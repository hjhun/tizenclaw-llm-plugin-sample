...
# TizenClaw Sample LLM Plugin Main Development Workflow

This workflow defines the core development process (Plan -> Develop -> Verify) for the `tizenclaw-llm-plugin-sample` project. The AGENT must always follow this process when performing tasks.
...
3. **Deploy and Install RPM Package**
   - We recommend using the `./deploy.sh` script to automate building and deployment.
   - If installing manually, push the built RPM to the device:
     - Command: `sdb push ~/GBS-ROOT/local/repos/tizen/<arch>/RPMS/tizenclaw-llm-plugin-sample-1.0.0-1.<arch>.rpm /tmp/`
     - Command: `sdb shell rpm -Uvh --force /tmp/tizenclaw-llm-plugin-sample-1.0.0-1.<arch>.rpm`
   - `<arch>` is automatically detected as the `cpu_arch` using `sdb capability` (e.g., `x86_64`, `aarch64`).

4. **Register Plugin with Unified Backend**
   - Command: `sdb shell unified-backend --preload -y org.tizen.tizenclaw-llm-plugin-sample`
   - This command registers the installed plugin application with the Application Framework.

5. **Restart Daemon and Verify Load Status**
   - Command: `sdb shell systemctl daemon-reload`
   - Command: `sdb shell systemctl restart tizenclaw`
   - Because the actual test involves TizenClaw loading the plugin's `.so` file, verification is done by checking the `TIZENCLAW` logs.
